...
public class Inventory : MonoBehaviour, ISaveManager
{
    ...
    [SerializeField] private Transform healPotionSlot;
    ...
    private void UpdateSlotUI()
    {
        ...
        foreach (KeyValuePair<ItemData_Equipment, InventoryItem> item in equipmentDictionary)
        {
            if (item.Key.equipmentType == EquipmentType.Flask)
                healPotionSlot.GetComponent<UI_EquipmentSlot>().UpdateSlot(item.Value);
        }
    ...
    public void UseFlask()
    {
        ...
        if (currentFlask == null)
            return;
        canUseFlask = Time.time > lastTimeUsedFlask + flaskCooldown;
        if (canUseFlask)
        {
            flaskCooldown = currentFlask.itemCooldown;
            currentFlask.Effect(null);
            lastTimeUsedFlask = Time.time;
            PlayerManager.instance.player.fx.HealingEffect();
            AudioManager.instance.PlaySFX(35, PlayerManager.instance.player.transform);
        }
        else
        {
            PlayerManager.instance.player.fx.CreatePopUpText("HealingOnCooldown", Color.red);
        }
    }
